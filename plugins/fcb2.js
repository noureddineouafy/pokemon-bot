...
if (!args[0]) throw 'هذا الامر خاص بالتحميل من الفيسبوك مثال': ${usedPrefix + command}* 'https://fb.watch/fOTpgn6UFQ/` 
if (!args[0].match(/www.facebook.com|fb.watch/g)) throw `*NOT A FB LINK, EXAMPLE: ${usedPrefix + command}* https://fb.watch/fOTpgn6UFQ/`
try {
m.reply(`*جاري تحميل الفيديو عليك بالصبر قليلا*`)    
...
await conn.sendFile(m.chat, url3, 'error.mp4', '*تم تحميل الفيديو بنجاح*\ninstagram.com/noureddine_ouafy', m)         
} catch (err5) {
console.log('5 ' + err5)    
try {
const { result } = await facebookdl(args[0]).catch(async _ => await facebookdlv2(args[0])).catch(async _ => await savefrom(args[0]))
for (const { url, isVideo } of result.reverse()) await conn.sendFile(m.chat, url, `facebook.${!isVideo ? 'bin' : 'mp4'}`, '*تم تحميل الفيديو بنجاح*\n instagram.com/noureddine_ouafy', m)    
} catch (err6) {
console.log('6 ' + err6)    
throw `*حذثت مشكلة*`
}}}}}}}
handler.command = /^(fcb2)$/i
