...
if (!args[0]) throw `*[❗𝐈𝐍𝐅𝐎❗] 𝙸𝙽𝙶𝚁𝙴𝚂𝙴 𝚄𝙽 𝙴𝙽𝙻𝙰𝙲𝙴 𝙳 𝙵𝙰𝙲𝙴𝙱𝙾𝙾𝙺, 𝙴𝙹𝙴𝙼𝙿𝙻𝙾: ${usedPrefix + command}* https://fb.watch/fOTpgn6UFQ/` 
if (!args[0].match(/www.facebook.com|fb.watch/g)) throw `*التحميل من فيسبوك, مثال: ${usedPrefix + command}* https://fb.watch/fOTpgn6UFQ/`
try {
m.reply(`*جاري التحميل انتظر قليلا لاتنسى متابعتي هنا ...* \n instagram.com/noureddine_ouafy`)      
const d2ata = await facebook.v1(args[0]);
let r2es = '';
if (d2ata.urls && d2ata.urls.length > 0) {
r2es = `${d2ata.urls[0]?.hd || d2ata.urls[1]?.sd || ''}`}
conn.sendFile(m.chat, r2es, 'error.mp4', `*instagram.com/noureddine_ouafy*`, m);
} catch (err1) {
console.log('1 ' + err1)      
try {
